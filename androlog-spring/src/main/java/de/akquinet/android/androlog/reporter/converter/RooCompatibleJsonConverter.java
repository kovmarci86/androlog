...
/**
 * This is a Duplication! of SimpleJsonConverter with simple modifications in its json format.
 * The json structure can be mapped to POJO's created by Spring Roo (FlexJson).
 * @author Marcell_Kovacs
 *
 */
public class RooCompatibleJsonConverter implements ReportConverter {
	...
	private void addApplicationData(JSONObject reportJson, Report report)
		...
		app.put("appPackage", context.getPackageName());
		try {
	...
	private void addReportHeader(JSONObject reportJson, Report report)
		...
		dates.put("dateSystem", System.currentTimeMillis());
		...
		if (err != null) {
			...
			if (err.getCause() != null) {
				...
				reportJson.put("causeStackTrace",
